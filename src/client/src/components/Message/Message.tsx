...
const Message = forwardRef<
    HTMLDivElement,
    {
        ...
        setMessages?: Dispatch<React.SetStateAction<IGetMessage[]>>;
    }
>(
    (
        { message, isSelf, setResend, isChild, deleteMessage, setMessages },
        ref
    ) => {
        const { store } = useContext(Context);

        const [childMessage, setChildMessage] = useState<JSX.Element | null>(
            null
        );
        ...
        useEffect(() => {
            ...
            if (message.senderId === store.user.userId) {
                ...
                socket.emit("subscribe_message_read", {
                ...
                socket.on("read_message", () => {
                    if (setMessages)
                        setMessages((prev) =>
                            prev.map((msgData) => {
                                if (msgData.messageId !== message.messageId)
                                    return msgData;
        ...
        return (
            <div
                ...
                className={classNames(s.message, {
                    ...
                    [s.unread]:
                        !isChild &&
                        message.senderId === store.user.userId &&
                        !message.isChecked,
                })}
            >
                {!isChild && (
